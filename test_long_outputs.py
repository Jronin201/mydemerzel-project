...
def test_long_output_streaming(monkeypatch):
    ...
    def fake_stream(messages, **kwargs):
        for ch in chunks:
            yield ('delta', ch)
        yield ('done', {'model':'gpt-5.2-long-test','id':'resp_long_stream','usage':{'input_tokens':900,'output_tokens':5000}})
        ...
        def fake_stream(messages, **kwargs):
            for i in range(near_cap_tokens):
                if i < 10:  # keep test runtime small by limiting actual yielded tokens
                    yield ('delta','Z')
            yield ('done', {'model':'gpt-5.2-nearcap','id':'resp_nearcap_stream','usage':{'input_tokens':1000,'output_tokens':near_cap_tokens},'near_cap':True,'truncated':False})
        monkeypatch.setattr(ai_client, 'request_stream', fake_stream)
        with app.test_client() as c:
    ...
    def test_truncated_streaming(monkeypatch):
        ...
        def fake_stream(messages, **kwargs):
            for i in range(10):
                yield ('delta','Q')
            yield ('done', {'model':'gpt-5.2-trunc','id':'resp_trunc_stream','usage':{'input_tokens':100,'output_tokens':cap},'near_cap':True,'truncated':True})
        monkeypatch.setattr(ai_client, 'request_stream', fake_stream)
        with app.test_client() as c:
