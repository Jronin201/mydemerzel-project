...
def test_token_error_then_retry(monkeypatch):
    ...
    def fake_request(messages, **kwargs):
        calls["n"] += 1
        if calls["n"] == 1:
            return {"output_text":"", "model":"gpt-5.2", "used_fallback": False, "error":"missing_output_text"}
        return {"output_text":"Recovered after token clamp", "model":"gpt-5.2", "used_fallback": False, "usage":{}, "id":"test"}
