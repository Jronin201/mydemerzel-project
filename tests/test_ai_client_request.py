...
class DummyResp:
    def __init__(self, output_text=None, model="gpt-5.2-2025-12-11", rid="resp_test_1", usage=None):
...
def test_returns_plain_text(monkeypatch):
    ...
    assert dummy.calls[0]["model"] == "gpt-5.2"
...
def test_log_observability_and_fallback(monkeypatch, capsys):
    ...
    log_text = captured
    assert "openai.model=gpt-5.2" in log_text
