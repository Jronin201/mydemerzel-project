...
def test_chat_route_mixed_history_non_stream(monkeypatch):
    ...
    with app.test_client() as c:
        ...
        data = resp.get_json()
        assert data['message']
        assert data['model'].startswith('gpt-5.2-') or 'gpt-5.2' in data['model']
...
def test_chat_route_mixed_history_stream(monkeypatch):
    ...
    def fake_stream(messages, **kwargs):
        yield ('delta','O')
        yield ('delta','K')
        yield ('done', {'model':'gpt-5.2-2025-12-11','id':'resp_stream_mixed','usage':{'input_tokens':20,'output_tokens':5}})
