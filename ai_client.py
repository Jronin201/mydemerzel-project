...
OPENAI_MODEL = os.getenv("OPENAI_MODEL", "gpt-5.3")  # Primary GPT-5.3 family model
...
MODEL_CONTEXT_WINDOW = int(os.getenv("MODEL_CONTEXT_WINDOW", "128000"))  # conservative GPT-5.3 context window default
        ...
        try:
            ...
            while True:
                ...
                if (not output_text) and idx == 0 and not primary_retry and not is_half_open:
                    # Responses API doesn't support reasoning parameter on gpt-5.3 or gpt-4o
                    # Skip reasoning retry and proceed to fallback if needed
                    print("[AI] skipping_reasoning_retry openai.model={mdl}".format(mdl=mdl))
                    primary_retry = True
                break
